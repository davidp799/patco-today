...
public class ParsePDF {
    ...
    public ArrayList<ArrayList<Trip>> getArrivalLines(int source_id, int destination_id) {
        ...
        for (String pdfLine : pdfLines) {
            processLine(pdfLine, source_id, destination_id, finalWestbound, finalEastbound);
        }
    ...
    // Extracted logic from the original for-loop into processLine
    private void processLine(String pdfLine, int source_id, int destination_id, ArrayList<Trip> finalWestbound, ArrayList<Trip> finalEastbound) {
        String newString = pdfLine;
        String[] badRegexValues = {"\\s", "\\t", "à", "Ã", "A", "P", "â", "–", "º"};
        String[] replacementValues = {"", "", "CLOSED,", "CLOSED,", "A,", "P,", "", "", ""};
        ...
        if (unformattedTrips.length >= 14) {
            ...
            if (formattedTrips.size() >= 14) {
                // Print all arrival times on one line, then the size
                StringBuilder sb = new StringBuilder();
                for (int i = 0; i < formattedTrips.size(); i++) {
                    sb.append(formattedTrips.get(i).getArrivalTime());
                    if (i < formattedTrips.size() - 1) {
                        sb.append(", ");
                    }
                }
                sb.append(" (").append(formattedTrips.size()).append(")");
